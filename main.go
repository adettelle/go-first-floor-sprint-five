...
func (r Running) Calories() float64 {
	a := CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift
	res := a * r.Weight / MInKm * float64(r.Duration.Hours()) * MinInHours
	return res
}
...
func (w Walking) Calories() float64 {
	...
	a := math.Pow(speedInMS, 2) / (w.Height / CmInM)
	b := CaloriesWeightMultiplier*w.Weight + a*CaloriesSpeedHeightMultiplier*w.Weight
	res := b * float64(w.Duration.Hours()) * MinInHours
	return res
}
...
func (s Swimming) Calories() float64 {
	a := s.meanSpeed() + SwimmingCaloriesMeanSpeedShift
	res := a * SwimmingCaloriesWeightMultiplier * s.Weight * float64(s.Duration.Hours())
	return res
}
